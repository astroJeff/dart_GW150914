...
model_kwargs = {'bhflag' : 3, 'acc_lim': 0, 'f_acc' : 0.5, 'cemergeflag' : 1}
...
def findStartPt(chains, derived):
    ...
    for star,ZAMS_star in zip(flat_derived,flat_chains): #broad filter on big mass range only
        ...
        if np.exp(ZAMS_star[2]) < 5e3 or ZAMS_star[3] > 0.6: continue 
...
with MPIPool() as pool:
    ...
    # Save the chains
    #np.save("data/150914/BHBH_chains_cemerge1.npy", pub.chains)
    #np.save("data/150914/BHBH_derived_cemerge1.npy", pub.derived)

    pub.chains = np.load("../data/150914/BHBH_chains_cemerge1.npy")
    pub.derived = np.load("../data/150914/BHBH_derived_cemerge1.npy") 
    ...
    # Save the chains
    np.save("../data/150914_PT/150914_chains_cemerge1_long_PT.npy", pub150914.chains)
    # np.save("data/150914/150914_derived_cemerge1_long.npy", pub150914.derived)
