...
model_kwargs = {'bhflag' : 3, 'acc_lim': 0, 'f_acc' : 0.5, 'qcrit_array' : 16*[2]}
...
def findStartPt(chains, derived):
    ...
    for star,ZAMS_star in zip(flat_derived,flat_chains): #broad filter on big mass range only
        ...
        if np.exp(ZAMS_star[2]) < 3e3 or ZAMS_star[3] > 0.6: continue
...
with MPIPool() as pool:
    ...
    #pub.aim_darts(N_iterations=200000)
    ...
    # Save the chains 
    #np.save("data/150914/BHBH_chains_qc2.npy", pub.chains)
    #np.save("data/150914/BHBH_derived_qc2.npy", pub.derived)

    pub.chains = np.load("../data/150914/BHBH_chains_qc2.npy")
    pub.derived = np.load("../data/150914/BHBH_derived_qc2.npy") 
    ...
    # Save the chains
    np.save("../data/150914_PT/150914_chains_qc2_long_PT.npy", pub150914.chains)
    # np.save("data/150914/150914_derived_qc2_long.npy", pub150914.derived)
